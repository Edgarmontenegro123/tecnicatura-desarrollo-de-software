...
class Reloj {
    ...
    mostrarTiempo() : string {
        const hh: string = this.horas < 10 ? `0${this.horas}` : `${this.horas}`;
        const mm: string = this.minutos < 10 ? `0${ this.minutos}` : `${this.minutos}`;
        const ss: string = this.segundos < 10 ? `0${this.segundos}` : `${this.segundos}`;
